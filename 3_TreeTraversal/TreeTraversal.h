...
class Tree {
 ...
 public:
  ...
  void external(TraversalType_e tt, Node *root1);
  void left(TraversalType_e tt, Node *root1);
  void below(TraversalType_e tt, Node *root1);
  void right(TraversalType_e tt, Node *root1);

  // Traverse the tree as per the given traversal type; this function must be recursive
  void traverse(TraversalType_e tt, Node *root1);
  Node *get_root();
};
